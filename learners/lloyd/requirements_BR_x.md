...
R1.6 | Bacteria must sense nutrients | Bacteria continue running along nutrient gradient.|Compare different layouts of nutrients | Business type | If the bacteria doesn't sense nutrients the run will cut short (no chemotaxis) | 5 | 4 | 20 |
|R1.7 | End condition | Program stops in organized manner and generates output. | Verify the output. Check that everything is cleaned up. | Business type | The program will not stop and no output | 5 | 5 | 25 |
| R1.7.1 | Run length | Gives run-length depending on nutrient intake | Confirm accurate run-length | Technical type | If the run length is not configure the nutrients intake has no influence to the bacteria movement | 3 | 3 | 9 |
...
| R11                 | Test-Driven Development  | Develop the project using TDD principles.| - Write tests before implementing each feature or functionality. <br /> - Ensure tests pass post-implementation.         |
